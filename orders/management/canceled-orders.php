...
$ordersdata = $orders->fetchOrders("where status='-1' $filteruser order by date desc LIMIT $inicio,$total_reg");
...
';
?>
<form action="canceled-orders.php" method="GET">
    <div class="row pd-2">
...
$orderdate = date("d/m/Y \à\s H:i:s a",strtotime($value['date']));
  ...
  print "<td>${value['pdescription']}</td>";
...
<div class="alert alert-success fade show" role="alert">
  <strong>VALOR TOTAL DOS PEDIDOS (exceto cancelados) = </strong> R$ '.floatval($totalValue).'
</div>
...
echo " <a class='btn red' href='canceled-orders.php?pagina=$anterior&last=$total_reg&filteruser=".$filteruserall."'><- Anterior</a> ";

  }else{
echo " <a class='btn red' href='canceled-orders.php?pagina=$anterior&last=$total_reg&filteruser=".$filteruserName."'><- Anterior</a> ";
  }
}else{
...
if ($pc<$tp) {
  if(isset($filteruserall)){
echo " <a class='btn green darken-2' href='canceled-orders.php?pagina=$proximo&last=$total_reg&filteruser=".$filteruserall."'>Próxima -></a>";

  }else{
echo " <a class='btn green darken-2' href='canceled-orders.php?pagina=$proximo&last=$total_reg&filteruser=".$filteruserName."'>Próxima -></a>";
  }
}else {
